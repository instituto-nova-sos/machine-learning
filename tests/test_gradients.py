"""Verificações numéricas das derivadas e do gradiente descendente."""
...
def test_finite_difference_matches_analytical_derivative() -> None:
    """A diferença central em ``x=3`` deve aproximar a derivada ``2x=6`` de ``x²``."""
    assert finite_difference(lambda x: x**2, 3.0) == pytest.approx(6.0, rel=1e-6)


def test_regression_weight_gradient() -> None:
    """Compara a derivada analítica do peso com uma perturbação numérica independente."""
    ...
    def loss_at_weight(weight: float) -> float:
        """Mantém o viés fixo e mede como somente o peso candidato altera o MSE."""
...
def test_gradient_descent_approaches_minimum() -> None:
    """Para ``f(x)=(x-3)²``, atualizações sucessivas devem se aproximar de ``x=3``."""
